"""VA-INV-001A — IAPD STATE/SEC census for Virginia.

Filter Virginia by registration/notice jurisdiction fields, never by address.
...
OUT = ROOT / "data" / "virginia" / "va-inv-001" / "iapd-va-census.json"
...
def parse_state(path: Path) -> dict:
    ...
    with gzip.open(path, "rb") as fh:
        for _, elem in iterparse(fh, events=("end",)):
                    ...
                    for wrap in list(child):
                        ...
                        for rg in list(wrap):
                            if local(rg.tag) != "Rgltr":
                                continue
                            if (rg.attrib.get("Cd") or "").upper() != "VA":
                                continue
            ...
            if crd and co_ia:
                state_ia[crd] = {**rec, "regs": co_ia, "jurisdiction": "VA"}
            if crd and co_era:
                state_era[crd] = {**rec, "regs": co_era, "jurisdiction": "VA"}
            if crd and main_st == "VA" and not co_ia and not co_era:
    ...
    return {
        ...
        "filter": "Rgltr/@Cd=VA (registration jurisdiction). Address is recorded only for overlap diagnostics.",
        "firms_in_feed": firms,
        ...
        "principal_office_co_among_state_ia": sum(1 for r in state_ia.values() if r["main_state"] == "VA"),
        "principal_office_not_co_among_state_ia": sum(1 for r in state_ia.values() if r["main_state"] != "VA"),
                ...
                if t == "Info":
                    ...
                    for st in list(child):
                        if local(st.tag) != "States":
                            continue
                        if (st.attrib.get("RgltrCd") or "").upper() != "VA":
                            continue
                        co_notice = {
...
def main() -> None:
        ...
        overlap_records.append(
            {
                ...
                "reading": "Both official 2026-08-27 compilations carry this exact firm CRD: APPROVED Virginia StateRgstn and FILED Virginia NoticeFiled. Credential classes remain separate. Populations are not required to be disjoint.",
            }
        )
    ...
    overlaps = {
        ...
        "state_ia_approved_and_notice_filed_joinMethod": "exact firm CRD set intersection of APPROVED StateRgstn/Rgltr/@Cd=VA and NoticeFiled/States/@RgltrCd=VA St=FILED",
        "state_ia_approved_and_notice_filed_records": overlap_records,
    }

    public = {
        "contract": "investor-va-iapd-census-v1",
        "safeAsCurrentVirginiaBulkStateLayer": True,
        "filterLogic": "Virginia selected by IAPD registration/notice jurisdiction fields (StateRgstn/Rgltr/@Cd, ERA/Rgltr/@Cd, NoticeFiled/States/@RgltrCd). Principal-office MainAddr/@State=VA is a separate national overlay and is not the state-RIA denominator.",
