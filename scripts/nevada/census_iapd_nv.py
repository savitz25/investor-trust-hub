"""NV-INV-001 — Nevada partition of the accepted IAPD compilation.
...
Lenses stay separate and are never summed:
  state IA        StateRgstn/Rgltr/@Cd=NV          (registration jurisdiction; status preserved)
  state ERA       ERA/Rgltr/@Cd=NV
  notice filing   NoticeFiled/States/@RgltrCd=NV    (SEC-registered or ERA firms; not state registration)
  principal office MainAddr/@State=NV               (geographic overlay; not registration)
...
OUT_DIR = ROOT / "data" / "nevada" / "nv-inv-001"
...
oh.CD = "NV"  # the Ohio parsers read the module-level jurisdiction code at call time
...
def main() -> None:
    ...
    state["filter"] = "Rgltr/@Cd=NV (registration jurisdiction). Address is recorded only for overlap diagnostics."
    ...
    def rename(d: dict) -> dict:
        return {k.replace("co_", "nv_", 1) if k.startswith("co_") else k.replace("_co_", "_nv_"): v
                for k, v in d.items() if not str(k).startswith("_")}

    census = {
        "contract": "investor-nv-iapd-census-v1",
        "ticket": "NV-INV-001",
        ...
        "filterLogic": "Nevada selected by IAPD registration/notice jurisdiction fields (StateRgstn/Rgltr/@Cd, ERA/Rgltr/@Cd, NoticeFiled/States/@RgltrCd). Principal-office MainAddr/@State=NV is a separate overlay and is not a registration denominator.",
        ...
        "lenses": {
            ...
            "state_address_without_nv_jurisdiction": state["address_co_without_co_jurisdiction"],
        },
        "overlaps": {
    ...
    (OUT_DIR / "iapd-nv-census.json").write_text(json.dumps(census, indent=2) + "\n", encoding="utf-8", newline="\n")
    (OUT_DIR / "iapd-nv-firm-crds.json").write_text(json.dumps({
