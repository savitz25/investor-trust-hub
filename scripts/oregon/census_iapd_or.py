"""OR-INV-001 — IAPD STATE/SEC census for Oregon.

Filter Oregon by registration/notice jurisdiction fields, never by address.
Does not scrape IAPD search. The 2026-08-27 compilation used by IL-INV-001 is
no longer retrievable (HTTP 403 AccessDenied). This census uses the next
official IAPD compilation that is deterministically downloadable:
IA_FIRM_STATE/SEC_Feed_09_10_2026.
"""
...
COMP_CANDIDATES = [
    ROOT / "data" / "raw" / "sec" / "form-adv" / "iapd-compilation-2026-09-10",
]
OUT = ROOT / "data" / "oregon" / "or-inv-001" / "iapd-or-census.json"
CD = "OR"
STATE_FEED = "IA_FIRM_STATE_Feed_09_10_2026.xml.gz"
SEC_FEED = "IA_FIRM_SEC_Feed_09_10_2026.xml.gz"
STATE_URL = "https://reports.adviserinfo.sec.gov/reports/CompilationReports/IA_FIRM_STATE_Feed_09_10_2026.xml.gz"
SEC_URL = "https://reports.adviserinfo.sec.gov/reports/CompilationReports/IA_FIRM_SEC_Feed_09_10_2026.xml.gz"
...
def parse_state(path: Path) -> dict:
    ...
    return {
        ...
        "filter": "Rgltr/@Cd=OR (registration jurisdiction). Address is recorded only for overlap diagnostics.",
        "firms_in_feed": firms,
...
def main() -> None:
        ...
        overlap_records.append(
            {
                ...
                "reading": "Both official 2026-09-10 compilations carry this exact firm CRD: APPROVED Oregon StateRgstn and FILED Oregon NoticeFiled. Credential classes remain separate. Populations are not required to be disjoint.",
            }
        )
    ...
    overlaps = {
        ...
        "state_ia_approved_and_notice_filed_joinMethod": "exact firm CRD set intersection of APPROVED StateRgstn/Rgltr/@Cd=OR and NoticeFiled/States/@RgltrCd=OR St=FILED",
        "state_ia_approved_and_notice_filed_records": overlap_records,
    }

    public = {
        "contract": "investor-or-iapd-census-v1",
        "safeAsCurrentOregonBulkStateLayer": True,
        "filterLogic": "Oregon selected by IAPD registration/notice jurisdiction fields (StateRgstn/Rgltr/@Cd, ERA/Rgltr/@Cd, NoticeFiled/States/@RgltrCd). Principal-office MainAddr/@State=OR is a separate national overlay and is not the state-RIA denominator.",
