"""MA-INV-001 — Massachusetts partition of the accepted IAPD compilation (2026-09-17).

No new ingestion pipeline: this reuses the OH-INV-001 parsers (scripts/ohio/census_iapd_oh.py)
on the byte-identical IA_FIRM_STATE / IA_FIRM_SEC feeds, filtered to registration jurisdiction MA.

Lenses stay separate and are never summed:
  state IA        StateRgstn/Rgltr/@Cd=MA          (registration jurisdiction; status preserved)
  state ERA       ERA/Rgltr/@Cd=MA
  notice filing   NoticeFiled/States/@RgltrCd=MA    (SEC-registered or ERA firms; not state registration)
  principal office MainAddr/@State=MA               (geographic overlay; not registration)
...
OUT_DIR = ROOT / "data" / "massachusetts" / "ma-inv-001"
...
oh.CD = "MA"  # the Ohio parsers read the module-level jurisdiction code at call time
...
def main() -> None:
    ...
    def rename(d: dict) -> dict:
        return {k.replace("co_", "ma_", 1) if k.startswith("co_") else k.replace("_co_", "_ma_"): v
                for k, v in d.items() if not str(k).startswith("_")}

    census = {
        "contract": "investor-ma-iapd-census-v1",
        "ticket": "MA-INV-001",
        "reuse": "OH-INV-001 parsers on the byte-identical accepted 2026-09-17 IAPD compilation (SHA-256 verified).",
        "filterLogic": "Massachusetts selected by IAPD registration/notice jurisdiction fields (StateRgstn/Rgltr/@Cd, ERA/Rgltr/@Cd, NoticeFiled/States/@RgltrCd). Principal-office MainAddr/@State=MA is a separate overlay and is not a registration denominator.",
        "state": rename(state),
        "sec": rename(sec),
        "lenses": {
            ...
            "state_address_without_ma_jurisdiction": state["address_co_without_co_jurisdiction"],
        },
        "overlaps": {
    ...
    (OUT_DIR / "iapd-ma-census.json").write_text(json.dumps(census, indent=2) + "\n", encoding="utf-8", newline="\n")
    (OUT_DIR / "iapd-ma-firm-crds.json").write_text(json.dumps({
