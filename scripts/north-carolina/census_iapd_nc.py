"""NC-INV-001 — IAPD STATE/SEC census for North Carolina.

Filter North Carolina by registration/notice jurisdiction fields, never by address.
Uses the latest official IAPD compilation available at acquisition:
IA_FIRM_STATE/SEC_Feed_09_17_2026.
"""
...
COMP_CANDIDATES = [
    ROOT / "data" / "raw" / "sec" / "form-adv" / "iapd-compilation-2026-09-17",
    Path(r"C:\Users\Michael.Savitsky\investor-pa-inv-001\data\raw\sec\form-adv\iapd-compilation-2026-09-17"),
]
OUT = ROOT / "data" / "north-carolina" / "nc-inv-001" / "iapd-nc-census.json"
CD = "NC"
STATE_FEED = "IA_FIRM_STATE_Feed_09_17_2026.xml.gz"
SEC_FEED = "IA_FIRM_SEC_Feed_09_17_2026.xml.gz"
STATE_URL = "https://reports.adviserinfo.sec.gov/reports/CompilationReports/IA_FIRM_STATE_Feed_09_17_2026.xml.gz"
SEC_URL = "https://reports.adviserinfo.sec.gov/reports/CompilationReports/IA_FIRM_SEC_Feed_09_17_2026.xml.gz"
...
def find_comp() -> Path:
    for path in COMP_CANDIDATES:
        if (path / STATE_FEED).is_file() and (path / SEC_FEED).is_file():
            return path
...
def parse_state(path: Path) -> dict:
    ...
    return {
        ...
        "filter": "Rgltr/@Cd=NC (registration jurisdiction). Address is recorded only for overlap diagnostics.",
        "firms_in_feed": firms,
...
def main() -> None:
    comp = find_comp()
    state_path = comp / STATE_FEED
    sec_path = comp / SEC_FEED
        ...
        overlap_records.append(
            {
                ...
                "reading": "Both official 2026-09-17 compilations carry this exact firm CRD: APPROVED North Carolina StateRgstn and FILED North Carolina NoticeFiled. Credential classes remain separate. Populations are not required to be disjoint.",
            }
        )
    ...
    overlaps = {
        ...
        "state_ia_approved_and_notice_filed_joinMethod": "exact firm CRD set intersection of APPROVED StateRgstn/Rgltr/@Cd=NC and NoticeFiled/States/@RgltrCd=NC St=FILED",
        "state_ia_approved_and_notice_filed_records": overlap_records,
    }

    public = {
        "contract": "investor-nc-iapd-census-v1",
        "safeAsCurrentNorth CarolinaBulkStateLayer": True,
        "filterLogic": "North Carolina selected by IAPD registration/notice jurisdiction fields (StateRgstn/Rgltr/@Cd, ERA/Rgltr/@Cd, NoticeFiled/States/@RgltrCd). Principal-office MainAddr/@State=NC is a separate national overlay and is not the state-RIA denominator.",
    ...
    (OUT.parent / "iapd-nc-ia-approved-crds.json").write_text(
        json.dumps({"crds": sorted(ia_approved)}, indent=2) + "\n", encoding="utf-8"
    )
    summary = {
        **public["overlaps"],
        "wrote": str(OUT),
