...
export async function executeParsedInvestorAsk(parsed: ParsedInvestorAsk, pageSize = INVESTOR_ASK_PAGE_SIZE): Promise<InvestorAskResult> {
  ...
  return {
    ...
    candidateSelection,
    answer: candidateSelection ? `Choose the exact firm before attaching the requested evidence. These are source-name candidates, not confirmed regulatory relationships.${total > 10 ? ' This bounded window shows at most 10 candidates. Refine the firm name to narrow the remaining matches.' : ''}` : q.answer,
    results,
    counts: [{ label: 'Matching firm facts', value: total, grain: 'form_adv_firm_facts rows matching filters' }],
    pagination: {
      page: candidateSelection ? 1 : q.page,
      pageSize: candidateSelection ? 10 : boundedPageSize,
      total,
      hasMore: !candidateSelection && q.page * boundedPageSize < total,
    },
