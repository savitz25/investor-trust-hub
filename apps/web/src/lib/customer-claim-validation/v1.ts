...
export type InvestorClaimValidationEnvelope = {
  ...
  publicationState: 'PUBLIC_CURRENT' | 'RESEARCH_ONLY' | 'UNPUBLISHED' | 'UNKNOWN';
  current: boolean;
...
function base(resultState: InvestorClaimValidationResultState): InvestorClaimValidationEnvelope {
  return {
...
export async function validateInvestorFirmClaim(input: unknown): Promise<InvestorClaimValidationEnvelope> {
  const parsed = investorClaimValidationRequestSchema.safeParse(input);
  if (!parsed.success) {
    return claimValidationError('INVALID_QUERY', 'invalid_claim_validation_request', parsed.error.issues.map((issue) => issue.message).join(' '));
  }
  ...
  const firm = await getFirmForClaimValidation(request.nativeProfileId, request.firmCrd);
  if (!firm) {
    return claimValidationError(
      'NO_CONFIDENT_MATCH',
  ...
  return {
    ...
    canonicalProfileUrl,
    regulatoryStatus: {
      firmClass: firm.report.classification.class === 'exempt_reporting_adviser' ? 'era' : 'ria',
