...
import { REGION_NAMES, type InvestorAskOverrides } from '@ith/domain';
...
export function InvestorSpecialistSearchShell({ query = '', compact = false, overrides = {} }: { query?: string; compact?: boolean; overrides?: InvestorAskOverrides }) {
  ...
  if(selected.firmType)selected.firmType=selected.firmType.toUpperCase();
    ...
    <form action="/ask" method="get" role="search" aria-label="Research investment adviser firms and Form ADV evidence" className="mt-5" data-specialist-search>
      ...
      <details className="mt-4 rounded-xl border border-[var(--ith-border)] bg-[var(--ith-canvas)] p-3">
        ...
        <div className="grid gap-4 pt-3 sm:grid-cols-2 lg:grid-cols-4">
          <Filter label="Firm class" name="firmType" value={selected.firmType} options={['Any explicit class', 'RIA', 'ERA']} />
          <Filter label="Principal-office state" name="state" value={selected.state} options={['Any sourced state', ...Object.keys(REGION_NAMES)]} />
          <Filter label="RAUM" name="raum" value={selected.raum} options={['Any reported RAUM', 'Reported zero', 'Under $25M', '$25M–under $100M', '$100M–under $1B', '$1B–under $10B', '$10B+']} />
          <Filter label="Compensation" name="compensation" value={selected.compensation} options={['Any reported method', 'Asset based', 'Hourly', 'Subscription', 'Fixed', 'Commission', 'Performance based']} />
        </div>
      </details>
    </form>
...
function Filter({ label, name, options, value }: { label: string; name: string; options: string[]; value?: string }) { return <label className="grid gap-1 text-sm font-semibold text-[var(--ith-navy)]">{label}<select name={name} defaultValue={value??options[0]} className="min-h-11 min-w-0 rounded-lg border border-[var(--ith-border)] bg-white px-3 font-normal">{options.map((x) => <option key={x} value={x === options[0] ? '' : x}>{x}</option>)}</select></label>; }
