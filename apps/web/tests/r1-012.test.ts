...
describe('R1-012 original research defects', () => {
  ...
  it.each(['Explain investment strategies','Tell me about returns','Describe my options'])('ordinary sentences are not bare firm names: %s',async(raw)=>{fixtureDb();const r=await executeInvestorAsk(raw);expect(r.parsed.query.nameQuery).toBeUndefined();expect(r.results).toEqual([]);expect(db.query).not.toHaveBeenCalled();});
  it.each(['Vanguard','Charles Schwab','Alpha Capital'])('recognizable bare firm names remain candidates: %s',(raw)=>{expect(interpretInvestorAskQuery(raw).query.nameQuery).toBe(raw);});
