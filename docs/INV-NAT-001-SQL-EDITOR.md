# INV-NAT-001B — Migration 0012 (applied)

**Production status:** applied. `schema_migrations` includes `0012_form_adv_enrichment.sql`.

Do **not** re-run this DDL unless a schema mismatch is proven. Wave 1 `search_documents.indexable` is not touched.

Canonical source: `database/migrations/0012_form_adv_enrichment.sql`.

Paste-ready copy of the applied statements:
...
SELECT count(*) FROM form_adv_reported_attributes;
SELECT count(DISTINCT firm_id) FROM form_adv_reported_attributes;
SELECT count(*) FROM form_adv_successor_links;
SELECT count(*) FROM search_documents WHERE entity_kind = 'firm' AND is_synthetic = false AND indexable;
-- expected: attributes 5149596, firms 23622, successor 16, indexable 1000
```
