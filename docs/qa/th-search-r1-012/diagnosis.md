...
Production certification correction: the independent row-source join established that current indexed firm facts use sec_ia_ria/sec_ia_era release label 2026-08-03, retrieved August 18, with NULL publication/checksum. The August 27 reference manifest is separate. See source-clock-followup.md and updated source-oracle.json. The final repair preserves those unknowns.

Final review found capitalized ordinary sentences could still pass the bare-name branch. The name boundary now requires an organization suffix or a short proper-name form, excluding initial research/help verbs. Explicit and quoted names remain supported; Vanguard, Charles Schwab and Alpha Capital are positive controls. Before observation is retained in name-boundary-before.json.
