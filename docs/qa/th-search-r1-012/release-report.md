...
| --- | --- |
| A. Status | RELEASED AND VERIFIED for the certified R1-012 cases only. |
| B. Model/reasoning | GPT-6 Astra / High - USER-CONFIRMED. No independently inspectable session metadata or claimed settings change. |
| C. Starting main | `e1f57a1b5092233892365f1c02a21b2cc935a2f5`; remote/AGENTS/worktrees/PRs/visible assignment ownership checked. Fresh isolated Investor branch. Other dirty worktrees untouched. |
| D. Historical QA | Portfolio, unspecified Form ADV/disclosure, Austin locality and Wyoming registration substitution still failed. Form ADV definition was already correct. |
| E. Root causes | Permissive name fallback; missing evidence identity requirement; state-first/overloaded geography; native English-appended filters; partial identifier capture; reference source date borrowed for unknown row dates. |
| F. Intent contract | Typed identifier/name/discovery/registration/Form ADV/disclosure/RAUM/explanation/advice-boundary/clarification operations refine the existing interpreter and executor. No LLM or competing runtime. |
| G. Name versus research | Research sentences are not default names. Explicit/structural names use bounded source predicates, actual matched field labels and revalidated candidate selection. Broad candidate windows request refinement rather than repeat pagination. |
| H. Identifier precedence | Full labeled CRD and supported SEC/file labels retain equality. Grouped CRD works; separate numeric ambiguity, attached junk and overlong input clarify/reject. A miss never broadens. |
| I. Registration jurisdiction | Retained separately from office state. WY approval is not established by the searchable operational SEC roster; no office substitute is executed. State/notice/ERA meanings remain explicit. |
| J. Principal-office geography | Exact recorded branch city and state predicates run before count/order/pagination. They do not establish service territory or registration jurisdiction. |
| K. City/state | Austin/TX is applied jointly. City-only requests retain city and offer a working state choice. Conflicts/multiple places remain unresolved. |
| L. RIA/ERA/notice/SEC | Current SEC registration uses regulator/type/status/current predicates; ERA and notice filings cannot satisfy state-RIA approval. Existing state intelligence remains separate. |
| M. Form ADV | Definition remains useful. Firm-specific questions require identity, then show existing published indexed fields and official full-record recovery; selected fields are not a complete filing. |
