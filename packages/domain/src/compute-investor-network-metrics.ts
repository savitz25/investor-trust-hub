...
export type InvestorNetworkMetricsInput = {
  ...
  nyPrincipalOfficeFirms: number;
  nyStateRiaApproved: number;
  nyNoticeFiled: number;
...
export function assertGrainSafety(input: InvestorNetworkMetricsInput): void {
  ...
  if (input.nyPrincipalOfficeFirms === input.rosterFirms) {
    throw new Error('NY principal-office overlay must not equal the national roster');
  }
  if (input.nyStateRiaApproved === input.nyPrincipalOfficeFirms) {
    throw new Error('NY state IA must not equal the principal-office overlay');
  }
  if (input.ilPrincipalOfficeFirms === input.rosterFirms) {
    throw new Error('IL principal-office overlay must not equal the national roster');
  }
...
export function computeInvestorNetworkMetrics(input: InvestorNetworkMetricsInput): InvestorNetworkMetricsV1 {
  ...
  const metrics: InvestorNetworkMetric[] = [
    ...
    metric({
      key: 'ny_state_ria_roster',
      label: 'New York state-registered investment-adviser firms',
      value: input.nyStateRiaApproved,
      valueState: 'KNOWN',
      grain: 'ny_state_ria_roster',
      denominator: 'IAPD state compilation APPROVED firms with registration jurisdiction = NY',
      description:
        'New York state-registered investment-adviser firms from IA_FIRM_STATE_Feed_08_27_2026. Not SEC RIA, not notice filing, not ERA, and not the 3,152 principal-office overlay.',
      coverage: 'New York',
      contributingSourceSystems: ['iapd_state_compilation'],
      sourceAsOf: input.publishedAt,
      generatedAt,
      publicationStatus: 'PUBLIC',
      trace: commonTrace(
        'IAPD StateRgstn/Rgltr/@Cd=NY and status APPROVED, counted as distinct firm CRD.',
        'Not SEC/IARD principal-office firms. Not federal-covered notice filings. Not state ERA reporting. Not IAR people.',
        ['iapd_state_compilation'],
        'New York',
        'IA_FIRM_STATE_Feed_08_27_2026',
    ...
    metric({
      key: 'il_state_ria_roster',
      label: 'Illinois state-registered investment-adviser firms',
      value: input.ilStateRiaApproved,
      valueState: 'KNOWN',
      grain: 'il_state_ria_roster',
      denominator: 'IAPD state compilation APPROVED firms with registration jurisdiction = IL',
      description:
        'Illinois state-registered investment-adviser firms from IA_FIRM_STATE_Feed_08_27_2026. Not SEC RIA, not notice filing, not ERA, and not the 793 principal-office overlay.',
      coverage: 'Illinois',
      contributingSourceSystems: ['iapd_state_compilation'],
      sourceAsOf: input.publishedAt,
      generatedAt,
      publicationStatus: 'PUBLIC',
      trace: commonTrace(
        'IAPD StateRgstn/Rgltr/@Cd=IL and status APPROVED, counted as distinct firm CRD.',
        'Not SEC/IARD principal-office firms. Not federal-covered notice filings. Not state ERA reporting. Not IAR people.',
        ['iapd_state_compilation'],
        'Illinois',
        'IA_FIRM_STATE_Feed_08_27_2026',
    ...
    metric({
      key: 'nv_state_ria_roster',
      label: 'Nevada state investment-adviser firms',
      value: input.nvStateRiaApproved,
      valueState: 'KNOWN',
      grain: 'nv_state_ria_roster',
      denominator: 'IAPD state compilation APPROVED firms with registration jurisdiction = NV',
      description:
        'Nevada state investment-adviser firms from IA_FIRM_STATE_Feed_09_17_2026 (NRS 90.330 calls this licensing). Not SEC RIA, not notice filing, not ERA, not TERMREQUEST rows, and not the 99 principal-office overlay.',
      coverage: 'Nevada',
      contributingSourceSystems: ['iapd_state_compilation'],
      sourceAsOf: input.publishedAt,
      generatedAt,
      publicationStatus: 'PUBLIC',
      trace: commonTrace(
        'IAPD StateRgstn/Rgltr/@Cd=NV and status APPROVED, counted as distinct firm CRD.',
        'Not SEC/IARD principal-office firms. Not federal-covered notice filings. Not state ERA reporting. Not IAR people. Not broker-dealers or sales representatives.',
        ['iapd_state_compilation'],
        'Nevada',
        'IA_FIRM_STATE_Feed_09_17_2026',
      ),
  ...
  const canonical = {
    ...
    nyHq: input.nyPrincipalOfficeFirms,
    nyStateRia: input.nyStateRiaApproved,
    ilHq: input.ilPrincipalOfficeFirms,
    ilStateRia: input.ilStateRiaApproved,
    ilNotice: input.ilNoticeFiled,
  ...
  return {
    ...
    newYork: {
      principalOfficeRosterFirms: input.nyPrincipalOfficeFirms,
      stateRiaRosterCoverage: 'ACQUIRED_IAPD_STATE_COMPILATION',
      statewideStateRiaUniverse: input.nyStateRiaApproved,
      noticeFiledFirms: input.nyNoticeFiled,
    },
    illinois: {
      principalOfficeRosterFirms: input.ilPrincipalOfficeFirms,
      stateRiaRosterCoverage: 'ACQUIRED_IAPD_STATE_COMPILATION',
    ...
    nevada: {
      principalOfficeRosterFirms: input.nvPrincipalOfficeFirms,
      stateRiaRosterCoverage: 'ACQUIRED_IAPD_STATE_COMPILATION',
