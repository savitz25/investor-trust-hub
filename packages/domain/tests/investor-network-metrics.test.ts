...
function baseInput(over: Partial<InvestorNetworkMetricsInput> = {}): InvestorNetworkMetricsInput {
  return {
    ...
    publishedStateIntelligencePaths: ['/new-jersey', '/california', '/texas', '/washington', '/arizona', '/colorado', '/virginia', '/new-york', '/illinois', '/oregon', '/pennsylvania', '/north-carolina', '/ohio', '/georgia', '/massachusetts', '/tennessee'],
    ...
    vaPrincipalOfficeFirms: 339,
    vaStateRiaApproved: 697,
    vaNoticeFiled: 3289,
    vaRegulatoryActivityRows: 146,
    nyPrincipalOfficeFirms: 3152,
    nyStateRiaApproved: 1297,
    nyNoticeFiled: 5856,
    ilPrincipalOfficeFirms: 793,
    ilStateRiaApproved: 855,
    ilNoticeFiled: 3560,
    orPrincipalOfficeFirms: 167,
    orStateRiaApproved: 335,
    orNoticeFiled: 2262,
    paPrincipalOfficeFirms: 623,
    paStateRiaApproved: 864,
    paNoticeFiled: 3411,
    ncPrincipalOfficeFirms: 325,
    ncStateRiaApproved: 701,
    ncNoticeFiled: 3704,
    ohPrincipalOfficeFirms: 426,
    ohStateRiaApproved: 784,
    ohNoticeFiled: 2733,
    maPrincipalOfficeFirms: 803,
    maStateRiaApproved: 773,
    maNoticeFiled: 3272,
    tnPrincipalOfficeFirms: 264,
    tnStateRiaApproved: 327,
    tnNoticeFiled: 2685,
    ...over,
  };
}
...
describe('investor-network-metrics-v1 grain safety', () => {
  ...
  it('does not coerce missing NJ/CA/TX/WA/AZ state-RIA universes to zero', () => {
    ...
    expect(metricByKey(m, 'published_state_intelligence_pages').value).toBe(16);
    ...
    expect(metricByKey(m, 'nc_state_ria_roster').value).toBe(701);
    expect(m.northCarolina.principalOfficeRosterFirms).toBe(325);
    expect(metricByKey(m, 'or_state_ria_roster').value).toBe(335);
    expect(m.oregon.principalOfficeRosterFirms).toBe(167);
    expect(metricByKey(m, 'pa_state_ria_roster').value).toBe(864);
    expect(m.pennsylvania.principalOfficeRosterFirms).toBe(623);
    expect(metricByKey(m, 'va_state_ria_roster').value).toBe(697);
    expect(metricByKey(m, 'ny_state_ria_roster').value).toBe(1297);
    expect(metricByKey(m, 'il_state_ria_roster').value).toBe(855);
    expect(m.illinois.principalOfficeRosterFirms).toBe(793);
    expect(m.newYork.principalOfficeRosterFirms).toBe(3152);
    expect(m.virginia.principalOfficeRosterFirms).toBe(339);
